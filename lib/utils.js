...
import { EOL } from 'os'
import { appConfigFile, dotenvFilename } from './constants.js'

export function atLeastOne (input) {
  if (input.length === 0) {
...
export function writeKeyAppConfig (generator, key, value) {
  const appConfigPath = generator.destinationPath(appConfigFile)
  writeKeyYAMLConfig(generator, appConfigPath, key, value)
}

// NOTE: supports setting multilayer keys like 'a.b.c=value'
export function writeKeyYAMLConfig (generator, configPath, key, value) {
...
export function writeMultiLayerKeyInObject (obj, key, value) {
...
export function readYAMLConfig (generator, configPath) {
...
export function writeYAMLConfig (generator, configPath, config) {
  generator.fs.write(configPath, yaml.dump(config))
}

export function guessProjectName (generator) {
...
export function readPackageJson (generator) {
  const packagejsonPath = generator.destinationPath('package.json')
  return generator.fs.readJSON(packagejsonPath) || {}
}

export function writePackageJson (generator, content) {
  const packagejsonPath = generator.destinationPath('package.json')
  return generator.fs.writeJSON(packagejsonPath, content || {})
}

export function addDependencies (generator, deps, dev = false) {
...
export function addPkgScript (generator, scripts) {
  const content = readPackageJson(generator)
  content.scripts = { ...content.scripts, ...scripts }
  writePackageJson(generator, content)
}

export function appendStubVarsToDotenv (generator, label, vars) {
  ...
  if (generator.fs.exists(file)) {
    prevContent = generator.fs.read(file)
  }
...
export function appendVarsToDotenv (generator, label, envVariable, value) {
  const searchString = envVariable
  ...
  if (prevContent.includes(envVariable)) {
    const content = `${envVariable}=${value}${EOL}`
    const formatted = prevContent.replace(re, content)
    generator.fs.write(file, formatted)
    return
  }
  const content = `## ${label}${EOL}${envVariable}=${value}${EOL}`
  generator.fs.append(file, content)
}
