...
export async function getEntitledProviderMetadataForOrg (eventsClient) {
...
export async function getProviderMetadata (obj, providerMetadataList, options) {
  const providerMetadataChoices = []
  for (const providerMetadata of providerMetadataList) {
    if (options?.providerMetadataId) {
  ...
  if (options?.providerMetadataId && providerMetadataChoices.length === 0) {
