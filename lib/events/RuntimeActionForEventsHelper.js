...
async function promptForRuntimeAction(obj) {
  ...
  if (!runtimeActionName) {
    runtimeActionName = await obj.promptForActionName(
        'showcases how to respond to IO Events', 'generic')
  }
  return runtimeActionName;
}

/** @private */
function getExistingActionNames (runtimeManifest, pkgName) {
  let actionNames = []
  let availableActions = runtimeManifest.packages[pkgName].actions
  for (let actionName in availableActions) {
    let actionDetails = availableActions[actionName]
    if(actionDetails.web === 'no' && actionDetails.annotations && actionDetails.annotations["require-adobe-auth"]===false)
