...
class EventsGenerator extends ActionGenerator {
  ...
  async promptForEventsDetails (defaultValues) {
    ...
    let runtimeActionName = await promptForRuntimeAction(this);
  ...
  addEvents (eventDetails, tplActionPath, options = {}) {
    ...
    let mapping = this.setDotEnvFileDetails(eventDetails)
  ...
  setDotEnvFileDetails(regDetails) {
    let providerMetadataToProvidersExistingMap = getProviderMetadataToProvidersExistingMap(this.providerMetadataToProviderMappingDotEnv)
    let newMapping = providerMetadataToProvidersExistingMap || {};
    console.log('New mapping before: ', newMapping)
    for (const providerMetadata of Object.keys(regDetails.selectedProvidersToEventMetadata)) {
      if(providerMetadataToProvidersExistingMap[providerMetadata] !== regDetails.selectedProvidersToEventMetadata[providerMetadata].provider.id) {
        console.log(`For provider metadata ${providerMetadata}, existing value ${providerMetadataToProvidersExistingMap[providerMetadata]} will be overwritten to ${regDetails.selectedProvidersToEventMetadata[providerMetadata].provider.id}`)
      }
