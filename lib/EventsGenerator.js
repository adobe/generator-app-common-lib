...
class EventsGenerator extends ActionGenerator {
  constructor (args, opts) {
    ...
    this.providerMetadataToProviderMapping = process.env.AIO_events_providermetadata_to_provider_mapping
  }
  ...
  setDotEnvFileDetails(regDetails) {
    ...
    for (const providerMetadata of Object.keys(regDetails.selectedProvidersToEventMetadata)) {
      if (mapping === '')
        mapping = mapping + providerMetadata + ':' + regDetails.selectedProvidersToEventMetadata[providerMetadata].provider.id
      else
        mapping = mapping + separator + providerMetadata + ':' + regDetails.selectedProvidersToEventMetadata[providerMetadata].provider.id
    }
    return mapping
  }
}
